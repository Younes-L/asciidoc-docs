...
// because this source document is also asciidoc, the following example can't display the escape backslash in the output, unless the directive syntax is purposefully wront.
// In this case, indenting the directive line by 1 space then setting the intent attribute to 0, results in the desired output.
